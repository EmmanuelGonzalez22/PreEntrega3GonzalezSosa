...
function actualizaDOMcarrito(json) {
  tbody.innerHTML = "";
  let html = "";
  ...
  for (const el of carritoSinDuplicados) {
    const seleccionado = json.filter((producto) => {
    ...
    html = `<tr class="row itemCarrito">
              ...
              <td class="col-1">
                <button class="btn btn-danger eliminarProducto" id="${seleccionado[0].id}">X</button>
              </td>
            </tr>`;
    ...
    removeItem.forEach((el) => {
      el.addEventListener("click", (e) => {
        eliminarProducto(e, json);
      });
    });

    // renderiza precio total
    calculaPrecioTotal(carrito, json);
  }
...
function agregarAlCarrito(json) {
...
// elimina producto del carrito
function eliminarProducto(event, json) {
...
function calculaPrecioTotal(arr, data) {
  const totalCarrito = document.querySelector("#totalCarrito");

  let total = arr
    .reduce((acc, el) => {
      const seleccionado = data.find((producto) => {
        return producto.id === parseInt(el);
      });
...
fetch(`./data/data.json`)
  ...
  .then((data) => {
    ...
    // LISTENER PARA FINALIZAR LA COMPRA
